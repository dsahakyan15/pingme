...
export const selectMessagesByType = createSelector(
  ...
  (messages, messageType) =>
    messages.filter(
      (m: WebSocketStoredMessage) =>
        m.kind === 'incoming' && (m as IncomingEnvelope).type === messageType
    )
);

// Domain chat messages (incoming only type==='message')
export const selectChatMessages = createSelector([selectWebSocketMessages], (messages) =>
  messages
    .filter(
      (m): m is WebSocketStoredMessage & IncomingEnvelope<Message> =>
        m.kind === 'incoming' && (m as IncomingEnvelope).type === 'message'
    )
    .map((m) => (m as unknown as IncomingEnvelope<Message>).data)
);

export const selectCurrentUser = createSelector([selectWebSocketState], (ws) => ws.currentUser);
...
export const makeSelectUsernameById = () =>
  createSelector(
...
export const selectIsLoadingHistory = (state: RootState): boolean =>
  state.websocket.isLoadingHistory || false;

// Conversation selectors
export const selectConversations = (state: RootState) =>
  Object.values(state.websocket.conversations || {});

export const selectActiveConversationId = (state: RootState) =>
  state.websocket.activeConversationId;

export const selectActiveConversation = (state: RootState) => {
  const activeId = state.websocket.activeConversationId;
  if (!activeId) return null;
  return state.websocket.conversations[activeId] || null;
};

export const selectConversationMessages = (state: RootState) => {
  const activeConversationId = state.websocket.activeConversationId;
  if (!activeConversationId) return [];

  return selectChatMessages(state).filter((msg) => msg.conversation_id === activeConversationId);
};
